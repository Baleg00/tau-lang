 ...
 */
#define TAU_ARGPARSE_OPTION(ID, SHORT_NAME, LONG_NAME, VALUE_NAME, DESCRIPTION)\
  {\
    .id          = (ID),\
 ...
 */
#define TAU_ARGPARSE_EOA (-1)

/**
 * \brief Identifier indicating an unknown argument.
 */
#define TAU_ARGPARSE_UNKNOWN (-2)

/**
 * \brief Represents an option.
 */
typedef struct tau_argparse_option_t
{
  ...
  int id;
} tau_argparse_option_t;

/**
 * \brief Represents an argument parser context.
 */
typedef struct tau_argparse_ctx_t tau_argparse_ctx_t;
