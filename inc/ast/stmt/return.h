 ...
 */
typedef struct tau_ast_stmt_return_t
{
  TAU_AST_STMT_HEADER;
  tau_ast_node_t* expr; // Pointer to the returned expression, or `NULL` in case of `unit`.
} tau_ast_stmt_return_t;
...
/**
 * \brief Performs control flow analysis pass on an AST return statement node.
 * 
...
/**
 * \brief Performs code generation pass on an AST return statement node.
 * 
