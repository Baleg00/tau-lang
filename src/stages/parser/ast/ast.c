...
ast_node_t* ast_node_init(ast_kind_t kind)
{
  ...
  case AST_TYPE_MEMBER:         node_size = sizeof(ast_type_mbr_t          ); break;
...
void ast_json_dump(FILE* stream, ast_node_t* node)
{
  ...
  case AST_TYPE_OPT:
    ...
    ast_json_dump(stream, ((ast_type_modif_t*)node)->base_type);
    break;
  case AST_TYPE_ARRAY:
    fprintf(stream, ",\"base_type\":");
  ...
  case AST_TYPE_FUN:
    ...
    fprintf(stream, ",\"callconv\":\"%s\"", callconv_kind_to_cstr(((ast_type_fun_t*)node)->callconv));
    break;
  case AST_TYPE_I8:
  case AST_TYPE_I16:
  case AST_TYPE_I32:
  ...
  case AST_TYPE_MEMBER:
    ...
    ast_json_dump(stream, ((ast_type_mbr_t*)node)->parent);
    fprintf(stream, ",\"member\":");
    ast_json_dump(stream, ((ast_type_mbr_t*)node)->member);
    break;
  case AST_TYPE_DECL:
    fprintf(stream, ",\"decl\":");
  ...
  case AST_EXPR_OP_UNARY:
    fprintf(stream, ",\"op_kind\":\"%s\"", op_kind_to_cstr(((ast_expr_op_un_t*)node)->op_kind));
    fprintf(stream, ",\"expr\":");
    ast_json_dump(stream, ((ast_expr_op_un_t*)node)->expr);
    break;
  case AST_EXPR_OP_BINARY:
    fprintf(stream, ",\"op_kind\":\"%s\"", op_kind_to_cstr(((ast_expr_op_bin_t*)node)->op_kind));
    fprintf(stream, ",\"lhs\":");
    ...
    break;
  case AST_EXPR_OP_CALL:
    fprintf(stream, ",\"op_kind\":\"%s\"", op_kind_to_cstr(AST_EXPR_OP_CALL));
  ...
  case AST_DECL_PARAM:
    ...
    fprintf(stream, ",\"is_vararg\":%s", ((ast_decl_param_t*)node)->is_vararg ? "true" : "false");
    break;
  case AST_DECL_FUN:
    fprintf(stream, ",\"id\":");
    ...
    fprintf(stream, ",\"callconv\":\"%s\"", callconv_kind_to_cstr(((ast_decl_fun_t*)node)->callconv));
    fprintf(stream, ",\"params\":");
...
void ast_json_dump_flat(FILE* stream, ast_node_t* node)
{
  ...
  while (!stack_empty(nodes))
  {
    ...
    if (!is_discovered)
    {
      ...
      case AST_TYPE_REF:
      case AST_TYPE_OPT:
        fprintf(stream, ",\"base_type\":\"%p\"", ((ast_type_modif_t*)node)->base_type);
        stack_push(nodes, ((ast_type_modif_t*)node)->base_type);
        break;
      case AST_TYPE_ARRAY:
      ...
      case AST_TYPE_UNIT:
        break;
      case AST_TYPE_MEMBER:
        fprintf(stream, ",\"parent\":\"%p\"", ((ast_type_mbr_t*)node)->parent);
        stack_push(nodes, ((ast_type_mbr_t*)node)->parent);
        fprintf(stream, ",\"member\":\"%p\"", ((ast_type_mbr_t*)node)->member);
        stack_push(nodes, ((ast_type_mbr_t*)node)->member);
        break;
      case AST_TYPE_DECL:
