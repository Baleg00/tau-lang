...
{
  tau_ast_type_ptr_t* node = (tau_ast_type_ptr_t*)malloc(sizeof(tau_ast_type_ptr_t));
  TAU_CLEAROBJ(node);

  tau_ast_registry_register((tau_ast_node_t*)node);

  node->kind = TAU_AST_TYPE_PTR;
...
void tau_ast_type_ptr_typecheck(tau_typecheck_ctx_t* ctx, tau_ast_type_ptr_t* node)
{
  ...
  TAU_ASSERT(tau_typedesc_can_add_ptr(base_desc));

  tau_typedesc_t* desc = tau_typebuilder_build_ptr(ctx->typebuilder, base_desc);
