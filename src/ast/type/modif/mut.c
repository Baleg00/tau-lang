...
{
  tau_ast_type_mut_t* node = (tau_ast_type_mut_t*)malloc(sizeof(tau_ast_type_mut_t));
  TAU_CLEAROBJ(node);

  tau_ast_registry_register((tau_ast_node_t*)node);

  node->kind = TAU_AST_TYPE_MUT;
...
void tau_ast_type_mut_typecheck(tau_typecheck_ctx_t* ctx, tau_ast_type_mut_t* node)
{
  ...
  TAU_ASSERT(tau_typedesc_can_add_mut(base_desc));

  tau_typedesc_t* desc = tau_typebuilder_build_mut(ctx->typebuilder, base_desc);
