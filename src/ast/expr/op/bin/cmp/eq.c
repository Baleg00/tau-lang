...
static void tau_ast_expr_op_bin_cmp_eq_typecheck_scalar(tau_typecheck_ctx_t* ctx, tau_ast_expr_op_bin_cmp_eq_t* node, tau_typedesc_t* lhs_desc, tau_typedesc_t* rhs_desc)
{
...
static void tau_ast_expr_op_bin_cmp_eq_typecheck_vector(tau_typecheck_ctx_t* ctx, tau_ast_expr_op_bin_cmp_eq_t* node, tau_typedesc_t* lhs_desc, tau_typedesc_t* rhs_desc)
{
...
static void tau_ast_expr_op_bin_cmp_eq_codegen_scalar(tau_codegen_ctx_t* ctx, tau_ast_expr_op_bin_cmp_eq_t* node)
{
  ...
  case OP_CMP_EQ_COMPLEX: node->llvm_value = tau_codegen_build_complex_eq(ctx, llvm_lhs_value, llvm_rhs_value); break;
  default: TAU_UNREACHABLE();
  }
}

static void tau_ast_expr_op_bin_cmp_eq_codegen_vector(tau_codegen_ctx_t* ctx, tau_ast_expr_op_bin_cmp_eq_t* node)
{
  tau_typedesc_t* lhs_desc = tau_typetable_lookup(ctx->typetable, node->lhs);
  tau_typedesc_t* rhs_desc = tau_typetable_lookup(ctx->typetable, node->rhs);
  ...
  node->llvm_value = tau_codegen_build_vector_eq(ctx, promoted_vec_desc, llvm_lhs_value, llvm_rhs_value);
}

tau_ast_expr_op_bin_cmp_eq_t* tau_ast_expr_op_bin_cmp_eq_init(void)
{
  tau_ast_expr_op_bin_cmp_eq_t* node = (tau_ast_expr_op_bin_cmp_eq_t*)malloc(sizeof(tau_ast_expr_op_bin_cmp_eq_t));
  TAU_CLEAROBJ(node);
...
void tau_ast_expr_op_bin_cmp_eq_nameres(tau_nameres_ctx_t* ctx, tau_ast_expr_op_bin_cmp_eq_t* node)
{
  tau_ast_node_nameres(ctx, node->lhs);
  tau_ast_node_nameres(ctx, node->rhs);
}

void tau_ast_expr_op_bin_cmp_eq_typecheck(tau_typecheck_ctx_t* ctx, tau_ast_expr_op_bin_cmp_eq_t* node)
{
  ...
  if (tau_typedesc_is_arithmetic(tau_typedesc_remove_ref_mut(lhs_desc)))
    tau_ast_expr_op_bin_cmp_eq_typecheck_scalar(ctx, node, lhs_desc, rhs_desc);
  else if (tau_typedesc_is_vector(tau_typedesc_remove_ref_mut(lhs_desc)))
    tau_ast_expr_op_bin_cmp_eq_typecheck_vector(ctx, node, lhs_desc, rhs_desc);
  else
    TAU_UNREACHABLE();
}

void tau_ast_expr_op_bin_cmp_eq_codegen(tau_codegen_ctx_t* ctx, tau_ast_expr_op_bin_cmp_eq_t* node)
{
  ...
  case OP_CMP_EQ_COMPLEX: tau_ast_expr_op_bin_cmp_eq_codegen_scalar(ctx, node); break;
  case OP_CMP_EQ_VECTOR: tau_ast_expr_op_bin_cmp_eq_codegen_vector(ctx, node); break;
  default: TAU_UNREACHABLE();
  }
}
