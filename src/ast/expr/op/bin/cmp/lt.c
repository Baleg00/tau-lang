...
tau_ast_expr_op_bin_cmp_lt_t* tau_ast_expr_op_bin_cmp_lt_init(void)
{
  tau_ast_expr_op_bin_cmp_lt_t* node = (tau_ast_expr_op_bin_cmp_lt_t*)malloc(sizeof(tau_ast_expr_op_bin_cmp_lt_t));
  TAU_CLEAROBJ(node);

  tau_ast_registry_register((tau_ast_node_t*)node);

  node->kind = TAU_AST_EXPR_OP_BINARY;
  node->op_kind = OP_CMP_LT;

  return node;
}

void tau_ast_expr_op_bin_cmp_lt_nameres(tau_nameres_ctx_t* ctx, tau_ast_expr_op_bin_cmp_lt_t* node)
{
  tau_ast_node_nameres(ctx, node->lhs);
  tau_ast_node_nameres(ctx, node->rhs);
}

void tau_ast_expr_op_bin_cmp_lt_typecheck(tau_typecheck_ctx_t* ctx, tau_ast_expr_op_bin_cmp_lt_t* node)
{
...
void tau_ast_expr_op_bin_cmp_lt_codegen(tau_codegen_ctx_t* ctx, tau_ast_expr_op_bin_cmp_lt_t* node)
{
  ...
  if (tau_typedesc_is_integer(promoted_desc))
  {
    ...
    {
      node->llvm_value = LLVMBuildICmp(ctx->llvm_builder, LLVMIntSLT, llvm_lhs_value, llvm_rhs_value, "");
    }
    else
    {
      node->llvm_value = LLVMBuildICmp(ctx->llvm_builder, LLVMIntULT, llvm_lhs_value, llvm_rhs_value, "");
    }
  }
  else if (tau_typedesc_is_float(promoted_desc))
  {
    node->llvm_value = LLVMBuildFCmp(ctx->llvm_builder, LLVMRealOLT, llvm_lhs_value, llvm_rhs_value, "");
  }
  else
  {
    TAU_UNREACHABLE();
