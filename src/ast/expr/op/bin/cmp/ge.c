...
{
  tau_ast_expr_op_bin_cmp_ge_t* node = (tau_ast_expr_op_bin_cmp_ge_t*)malloc(sizeof(tau_ast_expr_op_bin_cmp_ge_t));
  TAU_CLEAROBJ(node);

  tau_ast_registry_register((tau_ast_node_t*)node);

  node->kind = TAU_AST_EXPR_OP_BINARY;
  node->op_kind = OP_CMP_GE;
...
void tau_ast_expr_op_bin_cmp_ge_codegen(tau_codegen_ctx_t* ctx, tau_ast_expr_op_bin_cmp_ge_t* node)
{
  ...
  if (tau_typedesc_is_integer(promoted_desc))
  {
    ...
    {
      node->llvm_value = LLVMBuildICmp(ctx->llvm_builder, LLVMIntSGE, llvm_lhs_value, llvm_rhs_value, "");
    }
    else
    {
      node->llvm_value = LLVMBuildICmp(ctx->llvm_builder, LLVMIntUGE, llvm_lhs_value, llvm_rhs_value, "");
    }
  }
  else if (tau_typedesc_is_float(promoted_desc))
  {
    node->llvm_value = LLVMBuildFCmp(ctx->llvm_builder, LLVMRealOGE, llvm_lhs_value, llvm_rhs_value, "");
  }
  else
  {
    TAU_UNREACHABLE();
