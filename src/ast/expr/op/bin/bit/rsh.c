...
ast_expr_op_bin_bit_rsh_t* ast_expr_op_bin_bit_rsh_init(void)
{
  ast_expr_op_bin_bit_rsh_t* node = (ast_expr_op_bin_bit_rsh_t*)malloc(sizeof(ast_expr_op_bin_bit_rsh_t));
  CLEAROBJ(node);

  ast_registry_register((ast_node_t*)node);

  node->kind = AST_EXPR_OP_BINARY;
  node->op_kind = OP_BIT_RSH;

  return node;
}

void ast_expr_op_bin_bit_rsh_nameres(nameres_ctx_t* ctx, ast_expr_op_bin_bit_rsh_t* node)
{
  ast_node_nameres(ctx, node->lhs);
  ast_node_nameres(ctx, node->rhs);
}

void ast_expr_op_bin_bit_rsh_typecheck(typecheck_ctx_t* ctx, ast_expr_op_bin_bit_rsh_t* node)
{
...
void ast_expr_op_bin_bit_rsh_codegen(codegen_ctx_t* ctx, ast_expr_op_bin_bit_rsh_t* node)
{
