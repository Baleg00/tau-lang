...
{
  tau_ast_expr_op_bin_assign_arit_sub_t* node = (tau_ast_expr_op_bin_assign_arit_sub_t*)malloc(sizeof(tau_ast_expr_op_bin_assign_arit_sub_t));
  TAU_CLEAROBJ(node);
...
void tau_ast_expr_op_bin_assign_arit_sub_codegen(tau_codegen_ctx_t* ctx, tau_ast_expr_op_bin_assign_arit_sub_t* node)
{
  ...
  {
    node->llvm_value = tau_codegen_build_complex_sub(ctx, llvm_lhs_value, llvm_rhs_value);
  }
  else
  {
    TAU_UNREACHABLE();
