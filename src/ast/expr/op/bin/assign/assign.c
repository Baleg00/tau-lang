...
{
  tau_ast_expr_op_bin_assign_t* node = (tau_ast_expr_op_bin_assign_t*)malloc(sizeof(tau_ast_expr_op_bin_assign_t));
  TAU_CLEAROBJ(node);
...
void tau_ast_expr_op_bin_assign_typecheck(tau_typecheck_ctx_t* ctx, tau_ast_expr_op_bin_assign_t* node)
{
  ...
  if (!tau_typedesc_is_implicitly_direct_convertible(rhs_desc, tau_typedesc_remove_ref_mut(lhs_desc)))
...
void tau_ast_expr_op_bin_assign_codegen(tau_codegen_ctx_t* ctx, tau_ast_expr_op_bin_assign_t* node)
{
  ...
  LLVMBuildStore(ctx->llvm_builder, llvm_rhs_value, ((tau_ast_expr_t*)node->lhs)->llvm_value);
