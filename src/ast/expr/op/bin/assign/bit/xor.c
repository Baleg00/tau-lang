...
{
  tau_ast_expr_op_bin_assign_bit_xor_t* node = (tau_ast_expr_op_bin_assign_bit_xor_t*)malloc(sizeof(tau_ast_expr_op_bin_assign_bit_xor_t));
  TAU_CLEAROBJ(node);

  tau_ast_registry_register((tau_ast_node_t*)node);

  node->kind = TAU_AST_EXPR_OP_BINARY;
  node->op_kind = OP_ASSIGN_BIT_XOR;
...
void tau_ast_expr_op_bin_assign_bit_xor_codegen(tau_codegen_ctx_t* ctx, tau_ast_expr_op_bin_assign_bit_xor_t* node)
{
  ...
  node->llvm_value = LLVMBuildXor(ctx->llvm_builder, llvm_lhs_value, llvm_rhs_value, "");
