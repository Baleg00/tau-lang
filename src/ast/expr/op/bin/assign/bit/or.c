...
tau_ast_expr_op_bin_assign_bit_or_t* tau_ast_expr_op_bin_assign_bit_or_init(void)
{
  tau_ast_expr_op_bin_assign_bit_or_t* node = (tau_ast_expr_op_bin_assign_bit_or_t*)malloc(sizeof(tau_ast_expr_op_bin_assign_bit_or_t));
  TAU_CLEAROBJ(node);

  tau_ast_registry_register((tau_ast_node_t*)node);

  node->kind = TAU_AST_EXPR_OP_BINARY;
  node->op_kind = OP_ASSIGN_BIT_OR;

  return node;
}

void tau_ast_expr_op_bin_assign_bit_or_nameres(tau_nameres_ctx_t* ctx, tau_ast_expr_op_bin_assign_bit_or_t* node)
{
  tau_ast_node_nameres(ctx, node->lhs);
  tau_ast_node_nameres(ctx, node->rhs);
}

void tau_ast_expr_op_bin_assign_bit_or_typecheck(tau_typecheck_ctx_t* ctx, tau_ast_expr_op_bin_assign_bit_or_t* node)
{
...
void tau_ast_expr_op_bin_assign_bit_or_codegen(tau_codegen_ctx_t* ctx, tau_ast_expr_op_bin_assign_bit_or_t* node)
{
