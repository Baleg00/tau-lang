...
{
  tau_ast_expr_op_un_arit_inc_pre_t* node = (tau_ast_expr_op_un_arit_inc_pre_t*)malloc(sizeof(tau_ast_expr_op_un_arit_inc_pre_t));
  TAU_CLEAROBJ(node);

  tau_ast_registry_register((tau_ast_node_t*)node);

  node->kind = TAU_AST_EXPR_OP_UNARY;
  node->op_kind = OP_ARIT_INC_PRE;
...
void tau_ast_expr_op_un_arit_inc_pre_typecheck(tau_typecheck_ctx_t* ctx, tau_ast_expr_op_un_arit_inc_pre_t* node)
{
  ...
  tau_typetable_insert(ctx->typetable, (tau_ast_node_t*)node, expr_desc);
}
