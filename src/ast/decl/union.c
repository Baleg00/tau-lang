...
{
  tau_ast_decl_union_t* node = (tau_ast_decl_union_t*)malloc(sizeof(tau_ast_decl_union_t));
  TAU_CLEAROBJ(node);

  tau_ast_registry_register((tau_ast_node_t*)node);

  node->kind = TAU_AST_DECL_UNION;
...
void tau_ast_decl_union_typecheck(tau_typecheck_ctx_t* ctx, tau_ast_decl_union_t* node)
{
  ...
  tau_typedesc_t* desc = tau_typebuilder_build_union(ctx->typebuilder, (tau_ast_node_t*)node, field_types, field_count);
