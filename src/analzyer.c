...
ast_node_t* analyzer_visit_expr(analyzer_t* analyzer, symtable_t* table, ast_expr_t* node)
{
  ...
  case AST_ID:
  {
    ...
    symbol_t* id_sym = symtable_lookup(table, location_get_ptr(token_get_loc(id_tok)), location_get_len(token_get_loc(id_tok)));
...
ast_node_t* analyzer_visit_type_member(analyzer_t* analyzer, symtable_t* table, ast_type_member_t* node)
{
  ...
  symbol_t* owner_sym = symtable_lookup(table, location_get_ptr(token_get_loc(owner_id_tok)), location_get_len(token_get_loc(owner_id_tok)));
...
void analyzer_visit_decl_var(analyzer_t* analyzer, symtable_t* table, ast_decl_var_t* node)
{
  ...
  symbol_t* lookup = symtable_lookup(table, location_get_ptr(token_get_loc(id_tok)), location_get_len(token_get_loc(id_tok)));
  ...
  if (node->init != NULL)
  {
    ...
    if (!typedesc_is_same(init_desc, node->desc))
      report_error_type_mismatch(token_get_loc(node->tok), node->desc, init_desc);
  }
}
...
void analyzer_visit_decl_param(analyzer_t* analyzer, symtable_t* table, ast_decl_param_t* node)
{
  ...
  case AST_DECL_PARAM_DEFAULT:
    analyzer_visit_decl_param_default(analyzer, table, (ast_decl_param_default_t*)node);
    break;
  case AST_DECL_PARAM_VARIADIC:
    analyzer_visit_decl_param_variadic(analyzer, table, (ast_decl_param_variadic_t*)node);
    break;
  case AST_DECL_PARAM_GENERIC:
    analyzer_visit_decl_param_generic(analyzer, table, (ast_decl_param_generic_t*)node);
    break;
  default:
    node->type = analyzer_visit_type(analyzer, table, (ast_type_t*)node->type);
    node->desc = ast_desc_of(node->type);
...
void analyzer_visit_decl_enum_constant(analyzer_t* analyzer, symtable_t* table, symbol_t* enum_sym, ast_decl_enum_constant_t* node)
{
  ...
  token_t* id_tok = node->id->tok;
  symbol_t* enumerator_sym = symbol_init(location_get_ptr(token_get_loc(id_tok)), location_get_len(token_get_loc(id_tok)), (ast_node_t*)node);
